...
# Positionen
ABSENDER_TOP_START = (3 / 2.54) * 96
ABSENDER_TOP_END = (3.5 / 2.54) * 96
BETREFF_TOP = (7.5 / 2.54) * 96
KNICKFALTE_Y_POSITION = (6.5 / 2.54) * 96
AKTENZEICHEN_LEFT = (6 / 2.54) * 96
    ...
    try:
        with fitz.open(pdf_path) as pdf_document:
            ...
            if len(re.findall(r'\w+', os.path.basename(pdf_path))) > 2:
                betreff = re.findall(r'\w+', os.path.basename(pdf_path))[:4]
            else:
...
def extract_sender(page):
    ...
    for w in words:
        if ABSENDER_TOP_START <= w[1] <= ABSENDER_TOP_END:
            if not re.match(r'\d{5}', w[4]) and w[4] not in ignore_list:
                absender.append(w[4])
    return absender
